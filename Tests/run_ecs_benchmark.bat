...
rem
rem  usage:  run_ecs_benchmark.bat [debug^|release] [frames] [warmup]
rem          (default: release 300 30)
rem
...
set "WARMUP=%~3"
if "%WARMUP%"=="" set "WARMUP=30"

set "ROOT=%~dp0.."
set "OUTDIR=%~dp0build"
set "OBJDIR=%OUTDIR%\EcsBenchmark"
...
echo ----- building EcsBenchmark (%CONFIG%) -----
cl %COMMON_FLAGS% %CFG_FLAGS% ^
   /I "%ROOT%\Source" /I "%~dp0." ^
   /Fo"%OBJDIR%\\" /Fd"%OBJDIR%\EcsBenchmark.pdb" ^
   "%~dp0EcsBenchmark.cpp" "%~dp0BenchMeasure.cpp" %ENGINE_SOURCES% ^
   /Fe"%OUTDIR%\EcsBenchmark.exe"
if errorlevel 1 (
...
echo.
pushd "%ROOT%"
"%OUTDIR%\EcsBenchmark.exe" %FRAMES% %WARMUP%
set "RC=%errorlevel%"
popd
exit /b %RC%
...
set "VSPATH="
set "VSPATH_TMP=%TEMP%\glfd_ecsbench_vspath.txt"
