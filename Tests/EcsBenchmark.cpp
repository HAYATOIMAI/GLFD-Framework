...
int main(int argc, char** argv) {
  ...
  // **`return 0` にしない。** `~JobSystem` の停止経路には取りこぼしの余地が
  // あり、ここで固まることがある(1-5 の計測中に実測。CPU ほぼ 0 のまま
  // プロセスが終わらない)。ワーカーが `m_activeJobCount.wait(0)` へ入る
  // 直前に `Stop()` の `notify_all` が走ると通知を取りこぼし、
  // `std::jthread` の join が戻らなくなる形に見える。
  //
  // **これは計測ツールの都合で踏んだだけで、直すのは 1-5 の仕事ではない。**
  // ここで逃げているのは「基準線を取るために毎回 10 分待てない」ためであり、
  // **バグを消したのではなく避けているだけ**である。
  // 出力は上で流し終えているので、ここで打ち切っても結果は失われない。
  std::_Exit(0);
}
